    ...
    cart.forEach((item, index) => {
      ...
      list.innerHTML += `
        <div class="flex items-center gap-4 bg-gray-800 p-3 rounded-2xl border border-white/5">
          ...
          <button onclick="cart.splice(${index},1); renderCart(); updateUI();" class="text-red-500 px-2">✕</button>
        </div>`;
    });
...
let gameState = {
  ...
  currentEnergy: 100,
  tapPower: 1,
  upgradeCost: { power: 100, energy: 50 }
};

let gameEnergyRegenInterval;
...
function tapHamster() {
  if (gameState.currentEnergy <= 0) {
    alert("Energiya tugadi! Ozroq kutib oring.");
    return;
  }
  
  gameState.score += gameState.tapPower;
  gameState.currentEnergy = Math.max(0, gameState.currentEnergy - 10);
  
  // Level o'zgarish
  const levelUp = Math.floor(gameState.score / 50) + 1;
  if (levelUp > gameState.level) {
    gameState.level = levelUp;
    alert("🎉 Level " + gameState.level + " ga yetdingiz!");
  }
...
function buyUpgrade(type) {
  ...
  if (type === 'power') {
    ...
    alert("⚡ Kuch oshdi! Endi " + gameState.tapPower + " balladi bosite!");
  } else if (type === 'energy') {
    gameState.maxEnergy += 50;
    gameState.currentEnergy = Math.min(gameState.currentEnergy + 50, gameState.maxEnergy);
    alert("⚡ Energiya qo'shildi!");
  }
...
function updateGameUI() {
  document.getElementById('gameLevel').innerText = gameState.level;
  document.getElementById('gameScore').innerText = gameState.score;
  document.getElementById('gameEnergy').innerText = gameState.currentEnergy;
  
  const energyPercent = (gameState.currentEnergy / gameState.maxEnergy) * 100;
  document.getElementById('energyBar').style.width = energyPercent + '%';
  
  const nextLevel = gameState.level * 50;
  const progress = gameState.score - ((gameState.level - 1) * 50);
  const needed = 50 - progress;
  document.getElementById('levelProgress').innerText = `Level ${gameState.level + 1} gacha: ${Math.max(0, needed)} bosish kerak`;
  document.getElementById('tapCounter').innerText = `Jami bosish: ${gameState.score}`;
  
  // Hamster animatsiyasi
  const hamsterBtn = document.getElementById('hamsterBtn');
  hamsterBtn.classList.add('scale-110');
  setTimeout(() => hamsterBtn.classList.remove('scale-110'), 100);
}
...
function resetCombat() {
  ...
  document.getElementById('combatLog').innerHTML = '';
  document.getElementById('playerCharacter').innerText = '🐹';
  document.getElementById('enemyCharacter').innerText = '🐹';
}

function addDamageNumber(isPlayer, damage) {
  const container = document.getElementById('damageNumbers');
  const dmgEl = document.createElement('div');
  dmgEl.className = 'damage-number absolute font-bold text-xl pointer-events-none';
  dmgEl.innerText = `⚡ ${damage}`;
  dmgEl.style.cssText = isPlayer 
    ? 'left: 25%; top: 20px; color: #ef4444;' 
    : 'right: 25%; top: 20px; color: #ef4444;';
  container.appendChild(dmgEl);
  setTimeout(() => dmgEl.remove(), 900);
}

function shakeCharacter(isPlayer) {
  const el = isPlayer ? document.getElementById('playerCharacter') : document.getElementById('enemyCharacter');
...
function updateCharacterExpression(isPlayer, state) {
  const el = isPlayer ? document.getElementById('playerCharacter') : document.getElementById('enemyCharacter');
  if (state === 'hurt') {
    el.innerText = isPlayer ? '😵' : '😵';
    setTimeout(() => { el.innerText = '🐹'; }, 400);
  } else if (state === 'attack') {
    el.innerText = isPlayer ? '😠' : '😠';
    el.classList.add('spin-pulse');
    setTimeout(() => { 
      el.innerText = '🐹'; 
...
function playerAttack() {
  ...
  updateCharacterExpression(true, 'attack');
  shakeCharacter(false);
  addDamageNumber(false, dmg);
  
  logCombat(`⚔️ Siz hujum qildingiz — ${dmg} zarar! Combo: ${combatState.combo}`);
  updateCombatUI();
...
function enemyTurn() {
  ...
  updateCharacterExpression(false, 'attack');
  shakeCharacter(true);
  addDamageNumber(true, dmg);
  
  logCombat(`⚔️ Dushman hujum qildi — ${dmg} zarar`);
  updateCombatUI();
...
function updateCombatUI() {
  ...
  document.getElementById('playerHpLabel').innerText = `HP: ${combatState.playerHp}/${combatState.playerMax}`;
  document.getElementById('enemyHpLabel').innerText = `HP: ${combatState.enemyHp}/${combatState.enemyMax}`;
  document.getElementById('roundLabel').innerText = `Round ${combatState.round}`;
  document.getElementById('comboLabel').innerText = `Combo: ${combatState.combo}`;
  document.getElementById('specialBtn').disabled = !combatState.specialReady;
  document.getElementById('specialBtn').style.opacity = combatState.specialReady ? '1' : '0.6';
}
...
function endCombat(winner) {
  if (winner === 'player') {
    ...
    document.getElementById('playerCharacter').innerText = '🏆';
    document.getElementById('enemyCharacter').innerText = '😅';
    gameState.score += Math.floor(combatState.round * 10) + 50;
    updateGameUI();
  } else {
    document.getElementById('playerCharacter').innerText = '😅';
    document.getElementById('enemyCharacter').innerText = '😎';
...
window.onload = () => {
  ...
  document.getElementById('userName').addEventListener('keypress', (e) => { if (e.key === 'Enter') saveUser(); });
  document.getElementById('userPhone').addEventListener('keypress', (e) => { if (e.key === 'Enter') saveUser(); });
};
